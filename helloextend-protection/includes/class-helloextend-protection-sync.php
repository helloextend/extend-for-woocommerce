...
class HelloExtend_Protection_Sync
{
    ...
    public function hooks()
    {
        ...
        add_action('wp_ajax_helloextend_catalog_sync_reset', [ $this, 'helloextend_catalog_sync_reset' ], 10);
        add_action('wp_ajax_nopriv_helloextend_catalog_sync_reset', [ $this, 'helloextend_catalog_sync_reset' ], 10);
        add_action('wp_ajax_helloextend_catalog_sync_run', [ $this, 'helloextend_catalog_sync_run' ], 10);
        add_action('wp_ajax_nopriv_helloextend_catalog_sync_run', [ $this, 'helloextend_catalog_sync_run' ], 10);
    ...
    */
    public function helloextend_catalog_sync_reset()
    {
        ...
        $sync_options['helloextend_last_product_sync'] = null;
    ...
    */
    public function helloextend_catalog_sync_run()
    {
        ...
        check_ajax_referer('helloextend_sync_nonce', 'nonce');
        ...
        */
        ...
        // HelloExtend_Protection_Logger::helloextend_log_debug('Query :' .$products->request);
        ...
        if ($products->have_posts() ) {
            ...
            if (is_wp_error($response) ) {
                ...
                HelloExtend_Protection_Logger::helloextend_log_error(' Catalog Sync Batch #' . $batch_current . ' : POST request failed: ' . $error_message);
            } else {
                $response_code = wp_remote_retrieve_response_code($response);
                if ($response_code === 201 ) {
                    // success
                    HelloExtend_Protection_Logger::helloextend_log_notice('Catalog Sync Batch #' . $batch_current . ' was successful');
            ...
            HelloExtend_Protection_Logger::helloextend_log_error('No recently updated products found to sync.');
    ...
    */
    public function update_last_run_sync( $schedule = null )
    {
        ...
        $sync_options                             = get_option('helloextend_protection_for_woocommerce_catalog_sync_settings');
        $sync_time                                = time();
        $sync_options['helloextend_last_product_sync'] = $sync_time;
    ...
    {
        if ($this->settings['helloextend_use_special_price'] == '0' ) {
    ...
    {
        if ($this->settings['helloextend_use_skus'] == 1 ) {
    ...
    function sync_products_callback( $post_id, $post )
    {
        ...
        if ($this->settings['helloextend_sync_on_update'] == '1' ) {
            // Check if the post being updated is a woocommerce product.
            if ($post->post_type === 'product' ) {
                $this->helloextend_sync_one_product($post_id);
            }
        }
    }
    ...
    */
    public function helloextend_sync_one_product( $id )
    {
        /*
        ...
        */
        ...
        if ($product_query->have_posts() ) {
            while ( $product_query->have_posts() ) {
                ...
                if (is_wp_error($response) ) {
                    $error_message = $response->get_error_message();
                    HelloExtend_Protection_Logger::helloextend_log_error('Product Sync : POST request failed: ' . $error_message);
                } else {
                    $response_code = wp_remote_retrieve_response_code($response);
                    if ($response_code === 201 ) {
                        // success
                        ...
                        if (isset($data->added) && is_array($data->added) ) {
                            ...
                            if ($batch_added_count > 0 ) {
                                HelloExtend_Protection_Logger::helloextend_log_notice('Single Product Sync : item ID ' . $product_id . ' added');
                            }
                        }
                        ...
                        if (isset($data->updated) && is_array($data->updated) ) {
                            ...
                            if ($batch_updated_count > 0 ) {
                                HelloExtend_Protection_Logger::helloextend_log_notice('Single Product Sync : item ID ' . $product_id . ' updated');
                            }
                        }
                        ...
                        if (isset($data->errors) && is_array($data->errors) ) {
                            ...
                            if ($batch_errors_count > 0 ) {
                                HelloExtend_Protection_Logger::helloextend_log_notice('Single Product Sync : item ID' . $product_id . ' returned an error');
                            }
                        }
                    } //end response 201
    ...
    */
    public function sync_products_cron_job()
    {
        ...
        if ($this->settings['helloextend_automated_product_sync'] <> 'never' ) {
            HelloExtend_Protection_Logger::helloextend_log_notice('Running Scheduled Product Sync on the schedule called : ' . $this->settings['helloextend_automated_product_sync']);

            $batch_size = $this->settings['helloextend_sync_batch'];
            $args       = array(
                'post_type'      => 'product',
                'post_status'    => 'publish',
                'meta_query'     => array(
            ...
            for ( $page = 1; $page <= $batch_total; $page++ ) {
                ...
                if ($product_query->have_posts() ) {
                    while ( $product_query->have_posts() ) {
                        ...
                        if (is_wp_error($response) ) {
                            $error_message = $response->get_error_message();
                            HelloExtend_Protection_Logger::helloextend_log_error(' Catalog Sync Batch #' . $page . ' : POST request failed: ' . $error_message);
                        } else {
                            $response_code = wp_remote_retrieve_response_code($response);
                            if ($response_code === 201 ) {
                                // success
            ...
            if ($this->settings['enable_helloextend_debug'] == 1 ) {
                HelloExtend_Protection_Logger::helloextend_log_error('No recently updated products found to sync through schedule.');
            }
        }
    }
    ...
    private function process_response( $data, $batch_current = 0 )
    {
        ...
        if (isset($data->errors) && is_array($data->errors) ) {
            ...
            if ($batch_errors_count > 0 ) {
                ...
                HelloExtend_Protection_Logger::helloextend_log_error('Catalog Sync Batch #' . $batch_current . ', ' . $batch_errors_count . $items_errored . implode(',', $errors_item));
